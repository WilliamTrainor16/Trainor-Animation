...
// William Trainor 
class Snorlax extends Actor {
  // No new fields 
  
  public Snorlax(float xPos, float yPos){
  super( xPos, yPos);
  } // end Snorlax
...
public void capture(){
boolean xTouch = ((x < ball.x+7.5*ball.size) && (x > ball.x)); 
 boolean yTouch = ((y < ball.x+7.5*ball.size) && (y > ball.y)); 
  if (xTouch && yTouch) { 
    isVisible = false; 
    } // end if snorlaxVisible
  } // end capture
